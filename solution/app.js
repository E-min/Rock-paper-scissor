...
const action = (event) => {
    ...
    setTimeout(()=>{
        ...
        createPlayerOneSelected.setAttribute('class', `fa-regular ${playerOneSelectedHand} player-1`);
        playerOne.remove()
        containerHands.prepend(createPlayerOneSelected);

        const createPlayerTwoSelected = document.createElement('i');
        createPlayerTwoSelected.setAttribute('class', `fa-regular ${playerTwoSelectedHand} player-2`);
        playerTwo.remove()
        ...
        createRestartButton.setAttribute('class', 'fa-solid fa-arrow-rotate-left restart');
